...
def get_all_documents_by_etablissement(request, slug):
    ...
    for d in documents:
        data.append({
            ...
            'telecharge_par': d.telecharge_par.nom_complet if d.telecharge_par else None,
            'titre': d.titre,
...
def create_document(request, slug):
    ...
    telecharge_par_id = request.data.get('telecharge_par_id')
    ...
    if section_id:
        try:
            ...
            pass
    telecharge_par = None
    if telecharge_par_id:
        try:
            telecharge_par = Utilisateur.objects.get(id=telecharge_par_id, etablissement=etab)
        except Utilisateur.DoesNotExist:
            pass
    document = Document(
        etablissement=etab,
        section=section,
        telecharge_par=telecharge_par,
        titre=titre,
        fichier=fichier,
        tags=tags,
...
def update_document_by_id(request, slug, document_id):
    ...
    if telecharge_par_id:
        try:
            telecharge_par = Utilisateur.objects.get(id=telecharge_par_id, etablissement=etab)
            document.telecharge_par = telecharge_par
        except Utilisateur.DoesNotExist:
            document.telecharge_par = None
    ...
    document.type_fichier = request.data.get('type_fichier', document.type_fichier)
...
def get_document_by_id(request, slug, document_id):
    ...
    data = {
        ...
        'telecharge_par': document.telecharge_par.id if document.telecharge_par else None,
