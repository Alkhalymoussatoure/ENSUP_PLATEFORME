...
// Types pour l'intégration avec la modale
type Departement = { 
  id: number; 
  name: string; 
};
...
const MessageComposer: React.FC<MessageComposerProps> = ({
  ...
  // États locaux pour gérer les changements si les props ne sont pas fournies
  ...
  const fetchDepartements = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/${slug}/departements`, {
...
// 
  const fetchSections = async (departementId: string) => {
    try {
      const response = await fetch(`http://localhost:8000/api/departements/${departementId}/sections`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!response.ok) {
        throw new Error(`Erreur ${response.status}: ${response.statusText}`);
  ...
  // Fonction corrigée pour la sélection de département
  const onDepartementSelect = (id: string) => {
    setDepartementId(id);
    setMessageType('departement');
    fetchSections(id);
  };

  // Fonction corrigée pour la sélection de section
  ...
  const handleSearchUser = async (term: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/${slug}/users/search?query=${encodeURIComponent(term)}`, {
        headers: {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    localRecipients.split(',').forEach((matricule) => {
      const trimmedMatricule = matricule.trim();
      if (trimmedMatricule) {
        formData.append('destinataires', trimmedMatricule);
      }
    });

    if (messageType === 'section' && sectionId) {
      formData.append('section_id', sectionId);
    }
    if (messageType === 'departement' && departementId) {
      formData.append('departement_id', departementId);
    }

    try {
      const response = await fetch(`http://localhost:8000/api/${slug}/messages/send/`, {
        method: 'POST',
        body: formData,
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      alert("Échec de l'envoi du message. Vérifiez votre connexion.");
    }
  };
  ...
  // Fonction corrigée pour gérer la confirmation de la modale
  const handleModalConfirm = (users: User[]) => {
    const matricules = users.map(u => u.matricule).join(',');
    setLocalRecipients(matricules);
    if (onUpdateRecipients) {
      onUpdateRecipients(matricules);
    }
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
          <div className="flex items-center space-x-4">
            ...
            <label 
              htmlFor="file-input" 
              className="flex items-center space-x-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition cursor-pointer"
            >
            ...
            <div className="border border-gray-300 rounded-t-lg bg-gray-50 p-3">
              <div className="flex items-center space-x-3 flex-wrap">
                ...
                <button 
                  ...
                  className={`p-2 rounded ${textAlign === 'left' ? 'bg-blue-200' : 'hover:bg-gray-200'}`}
                >
                ...
                <button 
                  ...
                  className={`p-2 rounded ${textAlign === 'center' ? 'bg-blue-200' : 'hover:bg-gray-200'}`}
                >
                ...
                <button 
                  ...
                  className={`p-2 rounded ${textAlign === 'right' ? 'bg-blue-200' : 'hover:bg-gray-200'}`}
                >
